...
class PiSASR_eval(nn.Module):
    ...
    def forward(self, default, c_t, prompt=None):
        ...
        print(f"Encoded control dtype: {encoded_control.dtype}")
        model_pred = self._process_latents(encoded_control, prompt_embeds, default)


        # Decode output
        x_denoised = encoded_control - model_pred # N=1

        # x_denoised =  x_denoised - self._process_latents(x_denoised.to(torch.float16), prompt_embeds, default)  # N=2
        # x_denoised =  x_denoised - self._process_latents(x_denoised.to(torch.float16), prompt_embeds, default)  # N=3
        # x_denoised =  x_denoised - self._process_latents(x_denoised.to(torch.float16), prompt_embeds, default)  # N=4
        # x_denoised =  x_denoised - self._process_latents(x_denoised.to(torch.float16), prompt_embeds, default)  # N=5
        # x_denoised =  x_denoised - self._process_latents(x_denoised.to(torch.float16), prompt_embeds, default)  # N=6
    ...
    def _predict_no_tiling(self, encoded_control, prompt_embeds, default):
        ...
        if default:
            return self.unet(encoded_control,
                            self.timesteps1,
